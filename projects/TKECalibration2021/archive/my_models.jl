...
function build_loss(model, cdata)
        # Loss Function
        Nt = length(cdata.t)
        Δt = cdata.t[2] - cdata.t[1]
...
function conv_adj_independent_diffusivites(file)
    ...
    return MyModel(cdata, model, ParametersToOptimize, default_parameters, bounds)
end
