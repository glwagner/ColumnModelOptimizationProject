using ColumnModelOptimizationProject, Plots

# convert from offset arrays to normal arrays
cdata = ColumnData(files[1]*"/instantaneous_statistics.jld2")

# cdata = ColumnData("/Users/adelinehillier/.julia/dev/Data/WENO.jld2")
temp = cdata.T
Nz = cdata.grid.N
...
# 7e-4 strong wind
temperature(file) = [parent(x.data[1:Nz]) for x in ColumnData(file).T]
# uvelocity(file) = [parent(x.data[1:Nz]) for x in ColumnData(file).U]
# vvelocity(file) = [parent(x.data[1:Nz]) for x in ColumnData(file).V]
#
