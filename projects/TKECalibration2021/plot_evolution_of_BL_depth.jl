...
using Plots, PyPlot
using ColumnModelOptimizationProject: visualize_realizations, model_time_series
using Dao
...
for LEScase in values(LESdata)
    ...
    ℱ = model_time_series(best_parameters, md, cd)
    ...
    Plots.plot(days[toplot], h2_les[toplot], label = "LES mixed layer depth", linewidth = 3 , color = :red, grid = true, gridstyle = :dash, gridalpha = 0.25, framestyle = :box , title=cd.name, ylabel = "Depth [meters]", xlabel = "days", legend = :topleft)
    ...
    Plots.plot(days[toplot], h2_les[toplot], label = "LES mixed layer depth", linewidth = 3 , color = :red, grid = true, gridstyle = :dash, gridalpha = 0.25, framestyle = :box , title=cd.name, ylabel = "Depth [meters]", xlabel = "days", yscale = :log10, xscale = :log10, legend = :topleft)
    p = plot!(days[toplot], h2_model[toplot], color = :purple, label = "TKE mixed layer depth", linewidth = 3 )
    Plots.savefig(p, directory*cd.name*"_mixed_layer_depth_log10.pdf")

    p = visualize_realizations(md, cd, 1:180:length(cd), best_parameters)
