using Oceananigans, Printf, UnicodePlots, OceananigansAnalysis, Statistics
...
hmean(ϕ) = Array(dropdims(mean(data(ϕ), dims=(1, 2)), dims=(1, 2)))

function makeplot(model)
    Tavg = hmean(model.tracers.T)
    Uavg = hmean(model.velocities.u)
    Vavg = hmean(model.velocities.v)

    plot1 = lineplot(Tavg, model.grid.zC)

    plot2 = lineplot(Uavg, model.grid.zC)
    lineplot!(plot2, Vavg, model.grid.zC)
...
for i = 1:100
    ...
    #@time plot1, plot2 = makeplot(model)
end
