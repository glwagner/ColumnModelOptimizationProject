...
function FreeConvectionParameters()
    ...
    p_vary = (getproperty(parameters, name) for name in fieldnames(FreeConvectionParameters))
    FreeConvectionParameters(p_vary...)
end

function KPP_Parameters(p, K₀=nothing)
    param_dict = Dict((k, getproperty(params, k)) for k in propertynames(p))
